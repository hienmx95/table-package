...
import { applyDataCallServer, arrayMove, getColumnsInfo, getDefaultFilterData, getDefaultPagination, setUrlParams, getColumnsWidth } from './serviceWorker';
...
var usePrevPropsAndState = function usePrevPropsAndState(props, state) {
  var prevPropsAndStateRef = useRef({ props: null, state: null });
...
var useGetSnapshotBeforeUpdate = function useGetSnapshotBeforeUpdate(cb, props, state) {
  ...
  var componentJustMounted = useRef(true);
...
var TableCustom = function TableCustom(props) {
  ...
  var useComponentDidUpdate = useGetSnapshotBeforeUpdate(function (_, prevState) {
    return !isEqual(prevState.filterData, state.filterData) || !isEqual(prevState.searchValue, state.searchValue) || !isEqual(props.extendCondition, state.extendCondition);
  }, props, state);
  ...
  var fetchData = function fetchData() {
    var _getDefaultPagination = getDefaultPagination(props.history),
    ...
    setUrlParams({ history: history, filterData: filterData, searchValue: searchValue, extendCondition: extendCondition, fixedParam: fixedParam, current: current, pageSize: pageSize });
    var params = {
      ...
      return _regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.next = 2;
              return getColumnsInfo(props.code);
      ...
      case ACTIONS.ADD:
        {
          ...
          if (_type === TypeSearch.SEARCH) {
            if (!value.length) {
              delete temp[field];
            } else {
              temp[field] = value;
            }
          } else if (_type === TypeSearch.CHECKBOX || _type === TypeSearch.SEARCH_CHECKBOX) {
            if (!(field in temp)) {
              temp[field] = [];
            }
  ...
  var handleTableChange = function handleTableChange(pagination) {
    ...
    var queryData = applyDataCallServer(filterData);
    var history = props.history;
  ...
  var getTitlePopoverSearch = function getTitlePopoverSearch() {
    return props.columns.map(function (col) {
      ...
      if (!_.isEmpty(filterSource)) {
        return _extends({}, col, {
          title: React.createElement(PopoverSearch, {
  ...
  var getColumnsProps = function getColumnsProps() {
    ...
    var columnsWidth = getColumnsWidth(props.columns, [], widthScreen);
    var newColumns = columnsWithSearch.map(function (col, index) {
      ...
      var Title = function Title() {
        if (configTable) {
          return React.createElement(
            ...
            React.createElement(PopoverTableConfig, {
              columnsInfo: columnsInfo,
      ...
      return _extends({}, col, {
        ...
        ellipsis: false,
        title: !_.isEmpty(filterSource) ? title : React.createElement(Title, null)
      });
    }).filter(function (o) {
      return o.visible;
